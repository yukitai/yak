...
const get_span_info = (
    ...
    if (start.line === end.line) {
        ...
        const result = [
            {
                start: start.offset,
                len: src[line].length - start.offset,
            },
        ]
        ;++line
...
const visible_leading_whitespace = (s: string, c = '') => {
    ...
    return GRAY + '·'.repeat(split) + CLEAR + c +
        s.slice(split)
}
...
const render_preview = (
    ...
    const line_max = Math.min(
        src.length,
        end.line + preview_lines,
    )
    ...
    const render_line = (clip: boolean): number => {
        ...
        const content = visible_leading_whitespace(
            src[line],
        )
        result += `    ${GRAY}${BOLD}${
            align(align_max, (line + 1).toString())
        }${CLEAR} ${ITALLIC}${content}${CLEAR}\n`
        ;++line
        return jump
    }
    ...
    while (line < end.line) {
        ...
        if (span_info[i]?.len > 0) {
            result += `${
                align(
                    align_max + 5 + span_info[i].start,
                    '',
                )
            }${YELLOW}${'-'.repeat(span_info[i].len)}${CLEAR}\n`
        }
        i += 1
    }
    while (line < line_max) {
        ...
        switch (diff.operation) {
            case 'add':
                ...
                diff.value.forEach((v) => {
                    const vv = visible_leading_whitespace(
                        v,
                        GREEN,
                    )
...
export { type Difference, render_diff, render_preview }
