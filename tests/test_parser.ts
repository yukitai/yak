...
const assert_ast_eq = (struct: any, ast: any) => {
    if (struct instanceof Array) {
        ...
        struct.forEach((it: any, i: number) => {
            if (typeof it === 'object') {
                ...
                assertEquals(
                    ast[i],
                    it,
                    `${it.ty.name}: index ${i} not matched`,
                )
            }
        })
    } else {
        if (!(ast instanceof struct.ty)) {
            assert(
                false,
                `${struct.ty.name}: AST node not matched`,
            )
        }
        for (const key in struct) {
            ...
            if (typeof struct[key] === 'object') {
                ...
                assertEquals(
                    ast[key],
                    struct[key],
                    `${struct.ty.name}: \`${key}\` not matched`,
                )
            }
        }
    }
...
Deno.test({
    name: 'test_if_file_parser_works',
    fn() {
...
Deno.test({
    name: 'test_if_funcdef_parser_works',
    fn() {
        const context = new YakContext(
            `\
def b(a: ty, b) -> ty:
    ...`,
        ...
        const lexer = new Lexer(context)
        const parser = new Parser(lexer)
        const ast = parser.parse_func_definition()
...
Deno.test({
    name: 'test_if_type_parser_works',
    fn() {
        const context = new YakContext(
            `\
list<unsized [a; 1]>`,
...
Deno.test({
    name: 'test_if_block_parser_works',
    fn() {
        const context = new YakContext(
            `\
...
Deno.test({
    name: 'test_if_while_parser_works',
    fn() {
        const context = new YakContext(
            `\
while x:
...
Deno.test({
    name: 'test_if_forin_parser_works',
    fn() {
        const context = new YakContext(
            `\
for x in xs:
...
Deno.test({
    name: 'test_if_return_parser_works',
    fn() {
        const context = new YakContext(
            `\
return x`,
...
Deno.test({
    name: 'test_if_expr_parser_works',
    fn() {
        const context = new YakContext(
            `\
not a == b and 1 + 2 * 3 or (4 -7) ** 3`,
            'test',
        )
        const lexer = new Lexer(context)
        const parser = new Parser(lexer)
