...
const assert_token_eq = (ty: TokenType, value: string | number, token: Token) => {
...
Deno.test({
    ...
    fn () {
        const context = new YakContext("struct unsized use def if elif else while for in return", "test")
        const lexer = new Lexer(context)
...
Deno.test({
    ...
    fn () {
        const context = new YakContext("+-*/ // mod % ** and && or || not ! == != <> <=>= . =", "test")
        const lexer = new Lexer(context)
...
Deno.test({
    ...
    fn () {
        const context = new YakContext(`'hello world' "hello world" True False 0 0.1 0.`, "test")
        const lexer = new Lexer(context)
        assert_token_eq(TokenType.LiteralString, "hello world", (lexer.next_token() as Token))
        assert_token_eq(TokenType.LiteralString, "hello world", (lexer.next_token() as Token))
        assert_token_eq(TokenType.LiteralBool  , "True", (lexer.next_token() as Token))
        assert_token_eq(TokenType.LiteralBool  , "False", (lexer.next_token() as Token))
        assert_token_eq(TokenType.LiteralNumber, "0", (lexer.next_token() as Token))
        assert_token_eq(TokenType.LiteralNumber, "0.1", (lexer.next_token() as Token))
        assert_token_eq(TokenType.LiteralNumber, "0.", (lexer.next_token() as Token))
    }
})
